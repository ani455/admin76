...
export default function WithdrawRejectPage() {
  const { data: withdrawals, isLoading } = useQuery({
    queryKey: ["withdraw-rejected"],
    queryFn: () => remoteDb("get_withdraw_rejected"),
  });
    ...
    <div>
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-3 mb-6">
        <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: 'hsl(0, 72%, 51% / 0.12)', border: '1px solid hsl(0, 72%, 51% / 0.15)' }}>
          <XCircle className="w-5 h-5" style={{ color: 'hsl(0, 72%, 55%)' }} />
        </div>
        <div>
          <h2 className="text-lg font-bold text-foreground font-display">Withdraw Rejected</h2>
          <p className="text-[11px] text-muted-foreground">Rejected withdrawal requests</p>
        </div>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="glass-table rounded-2xl overflow-hidden">
        {isLoading ? (
          ...
          <div className="text-center py-20 text-muted-foreground text-sm">No rejected withdrawals</div>
        ) : (
